...
namespace SaaS.APIMis.Controllers.huitong
{
    public class HuiTongController : Controller
    {
        //
        // GET: /Huitong/
        public ActionResult payTo()
        {
            ...
            string payType = QueryStringHelper.GetString("payType");
            ...
            String PostUrl = "http://www.gtonepay.com/hspay/node";
            String p0_Cmd = "Buy";// 业务类型 是  Max(20)  固定值“Buy” . 1 

            string p1_MerId = "1093";// 商户编号 是  Max(11) 商户在汇通支付系统的唯一身份标识.获取方式见“如何获得商户编号” 2 
            string key = "JyObZprnbj118SjdE7FXsOHNU1S7vHI0"; //商户密钥

            String p2_Order = CodeHelper.OrderCode();//  商户订单号 是  Max(50) 若不为””，提交的订单号必须在自身账户交易中唯一;为 ””
            String p3_Amt = amount.ToString();//支付金额 是  Max(20)  单位:元，精确到分.此参数为空则无法直连(如直连会报错：抱歉，交易金额太小。),必须到汇通网关让消费者输入金额 4 
            String p4_Cur = "CNY";//  交易币种 是  Max(10)  固定值 ”CNY”. 5 
            String p5_Pid = "123";// 商品名称 是  Max(20) 用于支付时显示在汇通支付网关左侧的订单产品信息.
            String p6_Pcat = "123";// 商品种类 是  Max(20)  商品种类.
            String p7_Pdesc = "123"; //商品描述 是  Max(20)  商品描述.
            String p8_Url = "http://localhost/HuiTong/CallBack";// 商户接收支付成功数据的地址 是  Max(200) 支付成功后汇通支付会向该地址发送两次成功通知
            String pa_MP = "123";// 商户扩展信息 是  Max(200)  返回时原样返回，此参数如用到中文，请注意转码. 11 
            String pd_FrpId = payType;  //支付通道编码 否  Max(50) 默认为 ”” ，到汇通支付网关，汇通支付网关默认显示已开通的全部支付通道.
            String pr_NeedResponse = "1"; // 应答机制 是  Max(1) 固定值为“1”: 需要应答机制; 收到汇通支付服务器点对点支付成功通知，必须回写以”success”（无关大小写）开头的字符串，即使您收到成功通知时发现该订单已经处理过，也要正确回写”success”，否则汇通支付将认为您的系统没有收到通知，启动重发机制，直到收到”success”为止。 
            ...
            hmac = Encrypt.HmacSign(sbOld, key);
            ...
            string posturl = "";
            posturl += PostUrl;
            posturl += "?p0_Cmd=" + p0_Cmd;
            posturl += "&p1_MerId=" + p1_MerId;
            posturl += "&p2_Order=" + HttpUtility.UrlEncode(p2_Order, System.Text.Encoding.GetEncoding("gb2312"));
            posturl += "&p3_Amt=" + p3_Amt;
            posturl += "&p4_Cur=" + p4_Cur;
            posturl += "&p5_Pid=" + HttpUtility.UrlEncode(p5_Pid, System.Text.Encoding.GetEncoding("gb2312"));
            posturl += "&p6_Pcat=" + HttpUtility.UrlEncode(p6_Pcat, System.Text.Encoding.GetEncoding("gb2312"));
            posturl += "&p7_Pdesc=" + HttpUtility.UrlEncode(p7_Pdesc, System.Text.Encoding.GetEncoding("gb2312"));
            posturl += "&p8_Url=" + HttpUtility.UrlEncode(p8_Url, System.Text.Encoding.GetEncoding("gb2312"));
            posturl += "&pa_MP=" + HttpUtility.UrlEncode(pa_MP, System.Text.Encoding.GetEncoding("gb2312"));
            posturl += "&pd_FrpId=" + pd_FrpId;
            posturl += "&pr_NeedResponse=" + pr_NeedResponse;
            posturl += "&hmac=" + hmac;
            result.Message = PaySendBLL.PostData(posturl, "");
            // result.Code = payType;
           Response.Redirect(posturl);
            //if ((payType == "alipay"))//扫码支付
            //{
